(function () {

    "use strict";

    var fluid = require("infusion"),
        gpii = fluid.registerNamespace("gpii"),
        fs = require("fs");

    gpii.requests.request.handler.pcpMain = function(requestProxy, token, that) {
        that.options.token = token;
        requestProxy.events.onSuccess.fire(fs.readFileSync(__dirname + "/content/index.html").toString());

        /*// if using appjs:
        requestProxy.events.onSuccess.fire("personal control panel for " + token + " started\n" + __dirname);
        require("child_process").exec("node --harmony " + __dirname + "/app.js " + token);*/
    };

})();
