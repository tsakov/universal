...
var generateMainDialog = function (displayedPreferences) {
    var pcpMainDialogSource = fs.readFileSync("./content/deps/pcp_templates/pcpMainDialog.html", "utf8");

    jsdom.env({
        html: pcpMainDialogSource,
        scripts: ["http://code.jquery.com/jquery.js"],
        done: function (errors, domWindow) {
            var $ = domWindow.$;

            for (var i = 0; i < displayedPreferences.length; ++i) {
                currentPreference = displayedPreferences[i];
                $(".fl-uiOptions-category").append("<div class=\"pcp-" + currentPreference.preferenceName.split(" ").join("") + " fl-uiOptions-layout fl-col-flex\"> </div>\n");
            }
...
var generateIndex = function (displayedPreferences) {
    ...
    generateMainDialog(displayedPreferences);
};
